function combo_load(combo_id) {
    ...
    if(!combo.init) {
        ...
        for each (item_id in combo.items) {
            dojo.byId(item_id).value = "false";
            dojo.byId(item_id + "_img").src = combo.deselect_img;
        }
...
function combo_click(combo_id, item_id) {
    window.eval("var combo = " + dojo.byId(combo_id).value + ";");
    for each (item_id2 in combo.items) {
        dojo.byId(item_id2).value = "false";
        dojo.byId(item_id2 + "_img").src = combo.deselect_img;
    }
