...
<s:div cssClass="contentMain">
	<s:iterator value="getColors()">
		...
		<table width="99%">
			<tr>
				<td align="left">
				<h3><img id="color_<s:property value="getId()" />_img" onclick="coldiv_click('color_<s:property value="getId()"/>')" /> <img
					src="<s:property value="getImage_small()" />" width="20px" height="20px" /> <s:property value="getName(getLocale())" /></h3>
				</td>
				<td align="right">
				<form id="color_<s:property value="getId()"/>_form">
					<s:hidden name="colorFilter" value="%{getId()}"/>
					...
					new navRequest('catalogue','<s:property value="url_catalogue"/>',null,null,'color_<s:property value="getId()"/>_form'),
					new navRequest('content','<s:property value="url_content"/>',null,null,'color_<s:property value="getId()"/>_form')])"/>
				</form>
				</td>
			</tr>
		</table>
		...
		<table width="99%">
			<tr>
				<td><img src="<s:property value="getImage_medium()" />"
					onmouseover="show('<img src=<s:property value="getImage_large()"/> width=100px height=100px/>')" onmouseout="hide()" width="50px"
					height="50px" /></td>
				<td><s:property value="getDescription(getLocale())" /></td>
			</tr>
		</table>
		</div>
		<br clear="both" />
